...
fpga_result OPAE_SIMPLE_WRAPPER_allocate_io_buffers(fpga_handle accel_handle, 
	...
	ON_ERR_GOTO_local(__FILE__, __LINE__, res, out_release_input_buffer, "creating shared memory buffer fpgaPrepareBuffer (input)");
	res = fpgaGetIOAddress(accel_handle, *workspace_id_in, &io_addr_in);
	ON_ERR_GOTO_local(__FILE__, __LINE__, res, out_release_input_buffer, "creating shared memory buffer fpgaGetIOAddress (input)");

	// Output buffer
	res = fpgaPrepareBuffer(accel_handle, size_out, buf_addr_out, workspace_id_out, flag_input);
	ON_ERR_GOTO_local(__FILE__, __LINE__, res, out_release_output_buffer, "creating shared memory buffer fpgaPrepareBuffer (output)");
	res = fpgaGetIOAddress(accel_handle, *workspace_id_out, &io_addr_out);
	ON_ERR_GOTO_local(__FILE__, __LINE__, res, out_release_output_buffer, "creating shared memory buffer fpgaGetIOAddress (output)");

	// Load in AFU 
	res = fpgaWriteMMIO64(accel_handle, 0, HLS_MASTER_READ, io_addr_in);
	ON_ERR_GOTO_local(__FILE__, __LINE__, res, out_release_output_buffer,  "Load input data address");
	res = fpgaWriteMMIO64(accel_handle, 0, HLS_MASTER_WRITE, io_addr_out);
	ON_ERR_GOTO_local(__FILE__, __LINE__, res, out_release_output_buffer, "Load output data address");
...
fpga_result OPAE_SIMPLE_WRAPPER_call_afu(fpga_handle accel_handle, 
	...
	ON_ERR_GOTO_local(__FILE__, __LINE__, res, out_unregister_event, "Load CSR data");
	...
	ON_ERR_GOTO_local(__FILE__, __LINE__, res, out_destroy_handle, "error creating event accel_handle`");
	...
	ON_ERR_GOTO_local(__FILE__, __LINE__, res, out_unregister_event, "Could not register event");
	...
	do {
		...
		ON_ERR_GOTO_local(__FILE__, __LINE__, res, out_unregister_event, "checking Busy");
	...
	ON_ERR_GOTO_local(__FILE__, __LINE__, res, out_unregister_event, "trying to get file descriptor");
	...
	ON_ERR_GOTO_local(__FILE__, __LINE__, res, out_unregister_event, "Starting AFU");
	...
	if ( poll_res <= 0 ) {
		...
		ON_ERR_GOTO_local(__FILE__, __LINE__, res, out_unregister_event, "checking Done");

		// clear interrupt in component
		res = fpgaWriteMMIO64(accel_handle, 0, HLS_DONE_REG, (doneVal | HLS_INTERRUPT_STATUS_MASK));
		ON_ERR_GOTO_local(__FILE__, __LINE__, res, out_unregister_event, "Clearing interrupt");

	#ifdef DEBUG			
		if ( doneVal & HLS_DONE_MASK ) {				
			uint64_t outputData64;
			res = fpgaReadMMIO64(accel_handle, 0, HLS_RETURN_DATA, &outputData64);
			ON_ERR_GOTO_local(__FILE__, __LINE__, res, out_unregister_event, "reading return value");
